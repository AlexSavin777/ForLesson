...
class AddressBook:
    ...
    def initialization(self):
        ...
        self.status_bar = Label(main_frame, text="Savin *2024*", relief=SUNKEN, bd=1).pack(side=BOTTOM, fill=X)
    ...
    def save_address(self):
        outfile = open("K0Te.dat", "wb")
        pickle.dump(self.lst_addresses, outfile)
        tkinter.messagebox.showinfo("User Saved", "A new user have been saved")
        outfile.close()
    ...
    def load_address(self):
        if not os.path.isfile("K0Te.dat"):
            return []  # Return an empty list
        try:
            infile = open("K0Te.dat", "rb")
...
if __name__ == '__main__':
    ...
    application = AddressBook(root, "Test BD in Python")
